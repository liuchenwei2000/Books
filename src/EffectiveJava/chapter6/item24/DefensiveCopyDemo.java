 ...
 * 
 */
package chapter6.item24;

import java.util.Date;

/**
 * 保护性克隆(defensive copy)演示
 * 
 * @author 刘晨伟
 * 
 * 创建日期：2011-3-17
...
class Period2 {
	...
	public Period2(Date startDate, Date endDate) {
		...
		// 并且使用拷贝之后的对象作为实例的组件，而不使用原始的对象。
		this.startDate = new Date(startDate.getTime());
		this.endDate = new Date(endDate.getTime());
		// 保护性拷贝动作是在检查参数的有效性之前进行的，并且有效性检查是针对拷贝之后的对象，而不是原始对象。
		// 这样做是为了防止多线程环境中，另外一个线程会在恰好的时机改变原始的参数对象。
		if (this.startDate.compareTo(this.endDate) > 0) {
			throw new IllegalArgumentException(this.startDate + " after "
					+ this.endDate);
		}
	}

	/**
	 * 对于这两个访问方法是要使它们返回可变内部域的保护性拷贝即可。
	 */
	public Date getStartDate() {
